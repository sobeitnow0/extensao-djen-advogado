...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const MAPA_IBGE_OFFLINE_CAPITAIS = {
        'sao paulo': '3550308', 'rio de janeiro': '3304557', 'brasilia': '5300108',
        'salvador': '2927408', 'fortaleza': '2304400', 'belo horizonte': '3106200',
        'manaus': '1302603', 'curitiba': '4106902', 'recife': '2611606',
        'porto alegre': '4314902', 'belem': '1501402', 'goiania': '5208707',
        'guarulhos': '3518800', 'campinas': '3509502', 'sao luis': '2111300',
        'sao goncalo': '3304904', 'maceio': '2704302', 'duque de caxias': '3301702',
        'campo grande': '5002704', 'natal': '2408102', 'teresina': '2211005',
        'sao bernardo do campo': '3548708', 'nova iguacu': '3303500', 'joao pessoa': '2507507',
        'sao jose dos campos': '3549904', 'santo andre': '3547809', 'ribeirao preto': '3543402',
        'jaboatao dos guararapes': '2607901', 'osasco': '3534401', 'uberlandia': '3170206',
        'sorocaba': '3552205', 'contagem': '3118601', 'aracaju': '2800308',
        'feira de santana': '2910800', 'cuiaba': '5103403', 'joinville': '4209102',
        'juiz de fora': '3136702', 'londrina': '4113700', 'niteroi': '3303302',
        'anapolis': '5201108', 'florianopolis': '4205407', 'santos': '3548500',
        'vitoria': '3205309', 'macapa': '1600303', 'rio branco': '1200401',
        'boa vista': '1400100', 'palmas': '1721000', 'porto velho': '1100205'
    };

    async function buscarFeriadosMunicipaisAnual(ano, uf, municipioNome) {
        if (!uf || !municipioNome) return [];

        const normalizar = (txt) => txt.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9\s]/g, "").replace(/\b(de|do|da|dos|das)\b/g, "").replace(/\s+/g, " ").trim();
        const ufNorm = uf.toUpperCase().trim();
        const munNorm = normalizar(municipioNome);

        let codigoIbge = MAPA_IBGE_OFFLINE_CAPITAIS[munNorm];
        ...
        if (!codigoIbge) {
            try {
                ...
                const res = await window.promessasIbge[ufNorm];
                const mapaMunicipios = res ? res.mapa : {};
                const dados = res ? res.dados : [];
                codigoIbge = mapaMunicipios[munNorm];

                if (!codigoIbge && dados && dados.length > 0) {
                    const munLimpo = munNorm.split(' ')[0];
                    const encontrado = dados.find(m => {
                        const nNorm = normalizar(m.nome);
                        return nNorm.includes(munNorm) || munNorm.includes(nNorm) || (munLimpo.length >= 4 && nNorm.includes(munLimpo));
                    });
                    if (encontrado) codigoIbge = encontrado.id;
                }
            } catch(e) {
                console.warn("DJEN: Erro na busca de código IBGE:", e);
            }
        }
